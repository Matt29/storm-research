---
name: storm-research
description: Use when someone asks to run a Storm Research, use the storm-research skill, run the STORM method on a topic, says "storm research this" / "storm report on X" / "give me a STORM briefing on X", or wants a multi-perspective, citation-verified HTML research briefing. Runs a 4-phase pipeline, five expert lenses (Practitioner, Academic, Skeptic, Economist, Historian), then a contradiction map, then a synthesized HTML report, then adversarial peer review plus primary-source verification. Best for topics where multiple viewpoints and fact-checked claims matter, overkill for a simple factual lookup.
argument-hint: "[topic to research]"
---

# Storm Research

> English variant of `storm-recherche`. To use it, keep this as the skill's `SKILL.md` in a folder whose name matches the `name:` field above (`storm-research`), alongside `report-template.html`. Output is in English. The French version lives in `SKILL.md` of the `storm-recherche` folder.
...
This is a hand-built adaptation of Stanford's STORM method (Synthesis of Topic Outlines through Retrieval and Multi-perspective Question Asking, NAACL 2024). Difference to always keep in mind: the real STORM discovers its perspectives dynamically by surveying related articles, here the panel of five lenses is fixed by hand. It is an effective shortcut, not the original method. This honesty is structural, it is repeated in the report, never hidden.

## Honest broker positioning (non-negotiable)

This skill only exists if these three guardrails hold. Without them, it is not a Storm Research.

1. **Real research only.** Every lens and every citation must trace to a real, opened, read source. No invented studies, numbers, or URLs. If a figure cannot be verified, it is demoted or cut, never papered over.
2. **The panel is hand-built.** Always disclose this in the report. Agreement across lenses is a strong hypothesis, not independent proof, and never a consensus of the field.
3. **Verification is mandatory.** A report delivered without Phase 4 is not a Storm Research. The verification banner must be truthful (real count of fabricated / corrected / demoted).

## Portability

This skill is self-contained. It depends only on built-in Claude Code tools (the `Agent` tool with the built-in `general-purpose` agent, `Write`, and web search/fetch used inside those agents) plus `report-template.html` in this same folder. No external scripts, APIs, paid services, or other skills required. Drop the folder into any `.claude/skills/` directory and it works.

## Language and style

Output in English. Sober, factual tone, no overselling. No em dashes, use commas. HTML report brand: navy #0D1B2A, orange #F47C2B, cream #F5F5F0, Inter font. Keep the template CSS verbatim, do not switch to a different visual style. Note: the shipped `report-template.html` has French static labels, so when cloning it in Phase 3, translate the fixed section labels to English (see Phase 3).
...
3. Identify the **reader's role** so the actionable section can target it. Infer it from the topic and any stated context, if unclear ask in one line, or default to "a creator or decision-maker in this field".
...
When all five return, post a 2-3 line note in chat: which way they converge, and the sharpest disagreement. Keep the raw briefs out of chat (the agents already returned them).
...
1. Read `report-template.html` in this skill folder. Clone it, do not rebuild the CSS. The shipped template has French static labels, translate the fixed section labels to English as you fill it (for example: "Le résumé en 60 secondes" to "The 60-second summary", "Cinq enseignements clés, classés par fiabilité" to "Five key findings, ranked by reliability", "La connexion cachée" to "The hidden connection", "Ce qu'il faut vraiment faire différemment" to "What to actually do differently", "Ce que tu peux affirmer sans risque" to "What's safe to assert", "La question de frontière" to "The frontier question", "Comment lire ce rapport" to "How to read this", "Base de preuves · statut de vérification" to "Evidence base, verification status"). Keep the CSS untouched.
...
## Phase 4: adversarial peer review plus verification (do not skip)

This is what separates a Storm Research from a normal report. Run it before delivering.
...
2. Open it for the user with the platform's default opener: macOS `open <path>`, Linux `xdg-open <path>`, Windows `start "" <path>`. If the OS is unclear, just give the path.
...
- **Real research only.** See honest broker positioning. Never an invented source.
- **The panel is hand-built.** Always disclosed in the report. Convergence is a hypothesis, not a consensus of the field.
- **Verification is mandatory.** No Phase 4, no Storm Research. Banner must be truthful.
- **Reliability = evidence quality, not confidence.** Score on the hierarchy: peer-reviewed causal > official policy/financial data > single commissioned survey > analogy > preprint.
...
- **Design.** IA IRL brand, navy/orange/cream, Inter. Keep the template CSS verbatim. Do not swap in a different visual style.
